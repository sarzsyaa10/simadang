...
@php($pageTitle = 'Mutasi ' . ($kategori === 'peralatan' ? 'Peralatan' : 'Logistik Non Permakanan'))
...
@section('content')
    ...
    <div class="flex items-center gap-2 mb-5">
        <a href="{{ route('admin.stok.index', ['kategori' => $kategori]) }}"
    ...
    <div class="bg-white rounded-lg shadow p-5">
        ...
        <form method="GET" class="flex items-center gap-3 mb-4">
            ...
            <div class="relative w-full sm:w-72">
                <select name="gudang_id" onchange="this.form.submit()"
                        class="w-full appearance-none border border-gray-300 rounded px-3 py-2 text-sm bg-white">
                    <option value="">-- Semua Gudang --</option>
                    @foreach ($gudangList as $g)
                        <option value="{{ $g->id }}" @selected((string) $gudangId === (string) $g->id)>{{ $g->nama_gudang }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
            <a href="{{ route('admin.mutasi.create.masuk', ['kategori' => $kategori, 'gudang_id' => $gudangId]) }}"
            ...
            <a href="{{ route('admin.mutasi.create.keluar', ['kategori' => $kategori, 'gudang_id' => $gudangId]) }}"
        ...
        <div class="overflow-x-auto rounded border border-gray-100">
            <table class="w-full text-sm text-left">
                <thead class="bg-[#0f1f3d] text-white text-xs uppercase tracking-wide">
                    <tr>
                        <th class="px-4 py-3 whitespace-nowrap">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'tanggal', 'direction' => ($sort === 'tanggal' && $direction === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}"
                               class="inline-flex items-center gap-1.5 hover:text-blue-200 whitespace-nowrap">
                                Tanggal <x-icon name="bxs:sort-alt" class="w-4 h-4" />
                            </a>
                        </th>
                        <th class="px-4 py-3 whitespace-nowrap">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'gudang', 'direction' => ($sort === 'gudang' && $direction === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}"
                               class="inline-flex items-center gap-1.5 hover:text-blue-200 whitespace-nowrap">
                                Gudang <x-icon name="bxs:sort-alt" class="w-4 h-4" />
                            </a>
                        </th>
                        <th class="px-4 py-3 whitespace-nowrap">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'area', 'direction' => ($sort === 'area' && $direction === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}"
                               class="inline-flex items-center gap-1.5 hover:text-blue-200 whitespace-nowrap">
                                Arus <x-icon name="bxs:sort-alt" class="w-4 h-4" />
                            </a>
                        </th>
                        <th class="px-4 py-3 whitespace-nowrap">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'nama_barang', 'direction' => ($sort === 'nama_barang' && $direction === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}"
                               class="inline-flex items-center gap-1.5 hover:text-blue-200 whitespace-nowrap">
                                Nama Barang <x-icon name="bxs:sort-alt" class="w-4 h-4" />
                            </a>
                        </th>
                        <th class="px-4 py-3 whitespace-nowrap">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'jumlah', 'direction' => ($sort === 'jumlah' && $direction === 'asc') ? 'desc' : 'asc', 'page' => 1]) }}"
                               class="inline-flex items-center gap-1.5 hover:text-blue-200 whitespace-nowrap">
                                Qty <x-icon name="bxs:sort-alt" class="w-4 h-4" />
                            </a>
                        </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($mutasi as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2.5 text-gray-700 whitespace-nowrap">
                                ...
                                {{ \Illuminate\Support\Carbon::parse($item->jam)->format('H:i') }}
                            </td>
                            <td class="px-4 py-2.5 text-gray-700">{{ $item->gudang->nama_gudang ?? '-' }}</td>
                            ...
                            <td class="px-4 py-2.5 text-gray-700">{{ $item->barang->satuan ?? '-' }}</td>
                            <td class="px-4 py-2.5 text-gray-500">{{ $item->keterangan ?? '-' }}</td>
                            <td class="px-4 py-2.5">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.mutasi.edit', $item) }}" title="Edit"
                                    ...
                                    <form method="POST" action="{{ route('admin.mutasi.destroy', $item) }}" onsubmit="return confirm('Hapus data mutasi ini? Stok akan disesuaikan kembali.')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-10 text-center text-gray-400">Belum ada data mutasi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
