...
<div class="bg-white rounded shadow overflow-hidden">
    <div class="bg-[#0f1f3d] text-white text-sm font-semibold px-5 py-3">Ubah Data Gudang</div>

    <form method="POST" action="{{ route('admin.data-gudang.update', $gudang->id) }}" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="text" name="lokasi" value="{{ old('lokasi', $gudang->lokasi) }}" placeholder="Contoh: Jl. Merdeka No. 10, Cilacap"
        ...
        <div class="mb-6">
            ...
            <select name="operator_gudang_id" class="w-full border rounded px-3 py-2 text-sm">
                ...
                @foreach($operatorList as $op)
                    <option value="{{ $op->id }}" {{ old('operator_gudang_id', $gudang->operator_gudang_id) == $op->id ? 'selected' : '' }}>
                        {{ $op->nama }} ({{ $op->username }})
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex gap-3">
            ...
            <button type="submit" class="flex-1 bg-[#0f1f3d] hover:bg-[#16305e] text-white py-2 rounded text-sm">Ubah</button>
        </div>
    </form>
</div>
@endsection