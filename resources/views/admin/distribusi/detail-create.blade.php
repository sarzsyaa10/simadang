...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="bg-[#0f1f3d] text-white px-6 py-3.5">
        <h2 class="font-semibold">Tambah Data Distribusi Barang</h2>
        <p class="text-white/60 text-xs mt-0.5">No. Surat {{ $distribusi->nomor_surat }} &bull; Tujuan {{ $distribusi->tujuan ?: '-' }}</p>
    </div>

    <form method="POST" action="{{ route('admin.distribusi.detail.store', $distribusi->id) }}">
        @csrf
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" placeholder="Keterangan"
                    class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-900">{{ old('keterangan') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 p-4">
            ...
            <button type="submit"
                class="bg-[#0f1f3d] hover:bg-[#16295a] text-white font-semibold uppercase text-sm py-3.5 tracking-wide rounded-lg">Submit</button>
        </div>
    </form>
</div>
@endsection
