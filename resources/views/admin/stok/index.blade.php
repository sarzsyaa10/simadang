...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-5">
        ...
        <form method="GET" id="gudangForm" class="flex items-center gap-3 mb-4">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4">
            <a href="{{ route('admin.stok.create', ['kategori' => $kategori, 'gudang_id' => $gudangId]) }}"
            ...
            @if (Route::has('admin.mutasi.index'))
            ...
            <a href="{{ route('admin.stok.index', ['kategori' => $kategori, 'gudang_id' => $gudangId]) }}"
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody id="barang-table-body" class="divide-y divide-gray-100">
                    @forelse ($barang as $item)
                        @php($stokBarang = $item->stokBarang->first())
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2.5 text-gray-700">{{ $item->stok_total ?? 0 }}</td>
                            <td class="px-4 py-2.5 text-gray-700">{{ $item->satuan }}</td>
                            <td class="px-4 py-2.5">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.stok.edit', [$item, 'stok_id' => $stokBarang?->id]) }}"
                                       title="Edit"
                                    ...
                                    <form method="POST" action="{{ route('admin.stok.destroy', [$item, 'stok_id' => $stokBarang?->id]) }}" onsubmit="return confirm('Yakin hapus barang ini?')">
                                        @csrf @method('DELETE')
