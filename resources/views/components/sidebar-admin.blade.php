<aside
    ...
    <nav class="flex-1 px-2 py-4 space-y-1 text-sm overflow-y-auto"
        x-data="{
            openStok: {{ request()->routeIs('admin.stok.*') ? 'true' : 'false' }},
        ...
        <div>
            <button @click="openStok = !openStok"
                    class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.stok.*') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
            ...
            <div x-show="openStok" x-collapse x-cloak class="space-y-1 mt-1">
                <a href="{{ route('admin.stok.index', ['kategori' => 'logistik_non_permakanan']) }}"
                   @click="sidebarOpen = false"
                   class="block pl-9 pr-3 py-2 rounded {{ request()->routeIs('admin.stok.index') && (request('kategori', 'logistik_non_permakanan') === 'logistik_non_permakanan') ? 'bg-orange-500 font-semibold' : 'hover:bg-white/10' }}">
                ...
                <a href="{{ route('admin.laporan') }}"
                   @click="sidebarOpen = false"
                   class="block pl-9 pr-3 py-2 rounded {{ request()->routeIs('admin.laporan') ? 'bg-orange-500 font-semibold' : 'hover:bg-white/10' }}">
                    Laporan
                </a>
            </div>
        </div>
        ...
        <a href="{{ route('admin.permohonan') }}"
           @click="sidebarOpen = false"
           class="flex items-center gap-2.5 px-3 py-2 rounded {{ request()->routeIs('admin.permohonan') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
            <img src="{{ asset('images/icons/permohonan_bantuan.svg') }}" class="w-4 h-4" alt="">
            Permohonan Bantuan
        </a>
        <div>
            <button @click="openDistribusi = !openDistribusi"
                    class="w-full flex items-center justify-between px-3 py-2 rounded {{ request()->routeIs('admin.distribusi*') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
                <span class="flex items-center gap-2.5">
            ...
            <div x-show="openDistribusi" x-collapse x-cloak class="space-y-1 mt-1">
                ...
                @click="sidebarOpen = false"
                class="block pl-9 pr-3 py-2 rounded {{ request()->routeIs('admin.distribusi') ? 'bg-orange-500 font-semibold' : 'hover:bg-white/10' }}">
                    Distribusi
                </a>
                <a href="{{ route('admin.distribusi.pelaporan') }}"
                @click="sidebarOpen = false"
                class="block pl-9 pr-3 py-2 rounded {{ request()->routeIs('admin.distribusi.pelaporan') ? 'bg-orange-500 font-semibold' : 'hover:bg-white/10' }}">
        ...
        <a href="{{ route('admin.data-user.index') }}"
           @click="sidebarOpen = false"
           class="flex items-center gap-2.5 px-3 py-2 rounded {{ request()->routeIs('admin.data-user.*') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
        ...
        <a href="{{ route('admin.data-gudang.index') }}"
           @click="sidebarOpen = false"
           class="flex items-center gap-2.5 px-3 py-2 rounded {{ request()->routeIs('admin.data-gudang.*') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
        ...
        <a href="{{ route('admin.setting') }}"
           @click="sidebarOpen = false"
           class="flex items-center gap-2.5 px-3 py-2 rounded {{ request()->routeIs('admin.setting') ? 'bg-orange-500' : 'hover:bg-white/10' }}">
