...
@section('content')
    ...
    <div class="flex items-center gap-2 mb-5">
        <a href="{{ route('upt.stok.index', $kategori) }}"
    ...
    <div class="bg-white rounded-lg shadow w-full max-w-none overflow-hidden">
        <div class="bg-[#0f1f3d] text-white px-6 py-3.5">
            <h2 class="font-semibold">Tambah Barang Baru</h2>
        </div>

        <div class="p-5">
        <form method="POST" action="{{ route('upt.stok.store', $kategori) }}" enctype="multipart/form-data" class="space-y-4"
              x-data="{ preview: null }">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div x-data="{ satuan: '{{ old('satuan') }}' }">
                    ...
                    <input x-show="satuan === 'lainnya'"
                        type="text"
                        name="satuan_custom"
                        value="{{ old('satuan_custom') }}"
                        placeholder="Masukkan satuan..."
                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm mt-2 focus:outline-none focus:ring-2 focus:ring-blue-200">

                    <input type="hidden"
                        name="satuan"
                        :value="satuan === 'lainnya' ? '' : satuan">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Barang otomatis disimpan ke gudang UPT anda.</p>
            </div>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                          class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200">{{ old('deskripsi') }}</textarea>
            </div>
